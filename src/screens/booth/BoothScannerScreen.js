...
import { boothAdmission } from "../../api/Booth";
...
const BoothScannerScreen = ({ route, navigation }) => {
  const [permission, setPermission] = React.useState(null);
  const [scan, setScan] = React.useState(false);
  const [showModal, setShowModal] = React.useState(false);
  const [modalType, setModalType] = React.useState(null);
  const [message, setMessage] = React.useState("");
  const navigateBack = "Booth";
  const eventID = route.params?.eventID;
  const boothID = route.params?.itemID;
  ...
  const barCodeScan = ({ type, data }) => {
    ...
    boothAdmission(eventID, boothID, data)
      .then((result) => {
...
export default BoothScannerScreen;
